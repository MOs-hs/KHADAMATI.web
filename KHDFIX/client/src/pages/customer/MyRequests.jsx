...
const MyRequests = () => {
  ...
  useEffect(() => {
    ...
    setRequests(allRequests.filter(r => r.customer_id === user?.user_id));
  ...
  const getStatusInfo = (statusId) => {
    const statuses = {
      ...
      5: { label: 'Accepted', icon: FiClock, color: 'bg-blue-50 text-blue-700' }
    };
  ...
  const submitReview = async (e) => {
    ...
    console.log('Submitting review for request:', selectedRequest.request_id);
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        // Handle the Mock vs Real DB mismatch gracefully
        alert('The Request ID was not found in the database. (Note: Mock data requests cannot be reviewed in the real backend unless they execute sync logic).');
      } else {
