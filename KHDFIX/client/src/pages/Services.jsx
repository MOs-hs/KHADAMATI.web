...
import { getServices, getCategories, getProviders, getUsers, initializeMockData } from '../utils/mockData';
...
const Services = () => {
  ...
  useEffect(() => {
    ...
    // Filter by location if nearme is true
    if (nearmeParam === 'true' && latParam && lngParam) {
      // In production, filter by distance from user location
  ...
  const handleCategoryFilter = (categoryId) => {
    if (selectedCategory === categoryId) {
      ...
      setServices(getServices().filter(s => s.category_id === categoryId));
    }
  };
        ...
        {services.length > 0 ? (
          <div className="space-y-4">
            {services.map((service) => {
              const { provider, user } = getProviderInfo(service.provider_id);
